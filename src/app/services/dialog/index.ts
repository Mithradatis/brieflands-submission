...
import { nextStep, setFormStep } from '@/app/features/wizard/wizardSlice'
import {
...
const useHandleOperation = () => {
  ...
  const [getWorkflowTrigger] = useLazyGetWorkflowQuery();
  const [getSubmissionStepsTrigger] = useLazyGetSubmissionStepsQuery();
  const handleOperation = async (operation: string) => {
    switch (operation) {
      case 'delete-author':
        const response = await deleteAuthorTrigger(
          {
      ...
      case 'proceed-submission':
        ...
        await getWorkflowTrigger(dialog.actions.getWorkflow);
        await getSubmissionStepsTrigger(dialog.actions.getSubmissionSteps);
