...
import { handleInput, handleLoading } from '@/lib/features/submission/steps/comments/commentsSlice'
import { 
    getCommentsStepGuide, 
    getCommentsStepData, 
    updateCommentsStepData 
} from '@/lib/api/steps/comments' 

const CommentStep = forwardRef( ( prop, ref ) => {
    const dispatch: any = useDispatch();
    const formState = useSelector( ( state: any ) => state.commentsSlice );
    ...
    useEffect( () => {
        if ( wizard.formStep === 'comments' ) {
            dispatch( getCommentsStepData( getStepDataFromApi ) );
            dispatch( getCommentsStepGuide( getDictionaryFromApi ) );
    ...
    useImperativeHandle(ref, () => ({
        async submitForm () {
          ...
          let isAllowed = false;   
          try {
            await dispatch( updateCommentsStepData( getStepDataFromApi ) );
    ...
    return (
        <>
            ...
            <div id="comments" className={ `tab ${ ( formState.isLoading || typeof formState.stepGuide !== 'string' ) ? ' d-none' : ' d-block' }` }>
